...
#    */2 * * * *     root    /opt/pimp/cgmon.tcl >/dev/null 2>&1
...
# ASIC settings
set conf(cgminer_gpu_options) "-d gridseed -S gridseed:all --set-device gridseed:clock=850 --api-port 4029 --api-allow W:127.0.0.1"
...
set conf(mining_user) "root"

# Primary mining pool 
set pool(address1) "stratum+tcp://pool.ipominer.com:3333"
set pool(user1) "sling00.5"
set pool(pass1) "x"

# Additional mining pools (optional) 
set pool(address2) "stratum+tcp://pool.ipominer.com:3430"
set pool(user2) "sling00.5"
set pool(pass2) "5"
...
set conf(cgminer_exec) "bfgminer"
...
set conf(cgminer_path) "/opt/bfgminer/"
	...
	if {$cg_status == "child process exited abnormally"} {
		...
		if {$out == ""} {
			...
			if {$cg_status == "child process exited abnormally"} {
				...
				notice "$conf(cgminer_exec) failed to start.  Try running the mining command above to find the error.  Also, double check your BFGMINER options."	
		...
		if {![string match $argx "child process exited abnormally"]} {
			if {[string first "Connection refused" $argx] >1} {
				...
				notice "$conf(cgminer_exec) unknown API error.  Make sure $conf(cgminer_exec) is listening on 127.0.0.1:4029"
				exit
			}
			...
			if {[string match $argx "child process exited abnormally"]} {
				notice "$conf(cgminer_exec) is not responding.  Restarting miner."
				sendmail "[stamp] $hostname - $conf(cgminer_exec) is not responding - Restarting miner" "$hostname $conf(cgminer_exec) is not responding... restarting miner."			
				catch {exec echo "$conf(cgminer_exec) is not responding - Restarting miner in 10s..." | wall} out
				#reboot
				#insert custom code for bfgminer restart
				exec killall -9 bfgminer
				exec rm /tmp/accepted_count
				set exec_cmd "/tmp/cgmon-mine.sh"
				exec su $conf(mining_user) -c /bin/bash -c $exec_cmd
				#end custom code for bfgminer restart
				exit
			}
		}
...
#		}
				...
				if {$previous_time != ""} {
					...
					if {$elapsed_seconds > [expr $conf(share_timer) * 60]} {
						...
						if {$current_accepted($n) > $previous_accepted($n)} {
							notice [format "ASIC $n Shares accepted since last run:  [expr  $current_accepted($n) -  $previous_accepted($n)]  \(%.2f shares/min\)" $acc_rate_sec]
						} else {
							notice "ASIC $n no accepted shares in $elapsed_seconds seconds. ASIC probably hung."
							sendmail "[stamp] $hostname ASIC # $n - No shares" "$hostname - ASIC # $n no accepted shares in $elapsed_seconds seconds. ASIC probably hung."
							catch {exec echo "ASIC # $n No Shares - Rebooting..." | wall} out
							#reboot
							#insert custom code for bfgminer restart
							set exec_cmd "/tmp/cgmon-mine.sh"
							exec killall -9 bfgminer
		...
		notice "cgmon $conf(version) - $conf(cgminer_exec) running and all ASIC's healthy."
	}
}
